...
onMounted(async () => {
  try {
    ...
    console.log(attemptResponse.data.data)
    if (attemptResponse.data.data.status === 'DONE') {
      alert('이미 완료된 시험입니다.')
      if (window.opener) {
        window.opener.location.href = `/student/class-room/my-class`
        window.close()
      } else {
        router.push(`/student/class-room/my-class`)
      }
      return
    }
